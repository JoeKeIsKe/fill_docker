...
function Deposit() {
    ...
    useEffect(() => { 
        if (account ) { 
            FIT_contract.getBalance(account);
            FIT_contract.getCurrentRate()
    ...
    return <>
        <Header active={active} currentRate={ currentRate} onChange={(value) => { setActive(value)}}/>
        <div className='flex gap-x-4 mt-20'>
            <Banlance banlance={banlance_card.banlance} data={ 
                {
                    fil: FIL_banlance,
                    fit:FIT_banlance
                }
        ...
        <Card title={active === 'deposit' ? 'Deposit':'WidthDrow'} className='mt-20' >
        <div className="mt-10">
            <Chart option={default_opt }/>
            </div>
        </Card>
