...
export default () => {
  ...
  const renderCard = (dataItem: any) => {
    if (dataItem?.children?.length > 0) {
      return (
        <div className="bg-white px-5 pb-5 rounded-b-md">
                ...
                items={[
                  {
                        ...
                        {minerList.map((minerItem) => {
                          return (
                            <span className="flex gap-x-2">
                              <label className="text-gary-500">
                                {minerItem.title}:
                              </label>
                              <span className="font-meduim text-active">
                                {item[minerItem.dataIndex]}
                              </span>
                            </span>
                          );
                        })}
                    ...
                    children: <div>{renderCredit(item)}</div>,
                  },
                ]}
              />
            );
        ...
        <>
          ...
          <div className="body flex flex-col gap-y-5">
            {creditData?.children.map((record: any) => {
              return (
                <ul
                  className={`px-4 py-2 w-full  flex list-none rounded-lg cursor-pointer hover:bg-bgHover`}
                >
                  {creditList.map((item) => {
                    return (
      ...
      <div className="body flex flex-col gap-y-5">
        {data.map((record: any) => {
          return (
            <div>
              <ul
                className={`header px-4 py-4 w-full flex cursor-pointer  list-none rounded-lg bg-bgColor hover:bg-white ${
                  showPlus[record.key]
                    ? "bg-white rounded-t-md rounded-b-none"
                    : ""
                }`}
                onClick={() => {
                  setShowPlus({
                    ...showPlus,
                    [record.key]: !showPlus[record.key],
                  });
                }}
              >
